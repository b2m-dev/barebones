...
// in simulator we redefine to format for PFC Developer console
...
{\
	NSString * exceptionName = [@"org.pfc." stringByAppendingString:NSStringFromClass([self class])];\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
\
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.pfc";\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
...
enum {
	UIPFCNativeItemNone = -1, 
	UIPFCNativeItemSpinner = -2,
	UIPFCNativeItemProgressBar = -3,
	
	UIPFCNativeItemSlider = -4,
	UIPFCNativeItemSwitch = -5,
	UIPFCNativeItemMultiButton = -6,
	UIPFCNativeItemSegmented = -7,
	
	UIPFCNativeItemTextView = -8,
	UIPFCNativeItemTextField = -9,
	UIPFCNativeItemSearchBar = -10,
	
	UIPFCNativeItemPicker = -11,
	UIPFCNativeItemDatePicker = -12,
	
	UIPFCNativeItemInfoLight = -13,
	UIPFCNativeItemInfoDark = -14,
	
	UIPFCNativeItemDisclosure = -15,
	
	UIPFCNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all PFC instances of
