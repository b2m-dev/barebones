...
//Rare exceptions to indicate a bug in the _pfc code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_pfc = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_pfc]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.pfc.module"
